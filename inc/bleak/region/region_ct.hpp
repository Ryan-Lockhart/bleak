...
namespace bleak {
	template<typename T, extent_t RegionSize, extent_t ZoneSize, extent_t ZoneBorder> struct region_ct_t {
	  private:
		array_ct_t<zone_ct_t<T, ZoneSize, ZoneBorder>, RegionSize> zones;

	  public:
		using zone_type =zone_ct_t<T, ZoneSize, ZoneBorder>;
		...
		constexpr region_ct_t() : zones{} {};

		constexpr region_ct_t(cref<std::string> path) noexcept : zones{} {
		...
		constexpr region_ct_t(cref<region_ct_t> other) : zones{ other.zones } {}

		constexpr region_ct_t(rval<region_ct_t> other) noexcept : zones{ std::move(other.zones) } {}

		constexpr ref<region_ct_t> operator=(cref<region_ct_t> other) noexcept {
			if (this != &other) {
				zones = other.zones;
			}
		...
		constexpr ref<region_ct_t> operator=(rval<region_ct_t> other) noexcept {
			if (this != &other) {
		...
		constexpr ~region_ct_t() noexcept {};

		constexpr ref<zone_ct_t<T, ZoneSize, ZoneBorder>> operator[](offset_t::product_t position) noexcept { return zones[position]; }

		constexpr cref<zone_ct_t<T, ZoneSize, ZoneBorder>> operator[](offset_t::product_t position) const noexcept { return zones[position]; }

		constexpr ref<zone_ct_t<T, ZoneSize, ZoneBorder>> operator[](offset_t position) noexcept { return zones[position]; }

		constexpr cref<zone_ct_t<T, ZoneSize, ZoneBorder>> operator[](offset_t position) const noexcept { return zones[position]; }

		constexpr ref<zone_ct_t<T, ZoneSize, ZoneBorder>> operator[](offset_t::product_t zone_position, offset_t::product_t cell_position) noexcept { return zones[zone_position][cell_position]; }

		constexpr cref<zone_ct_t<T, ZoneSize, ZoneBorder>> operator[](offset_t::product_t zone_position, offset_t::product_t cell_position) const noexcept { return zones[zone_position][cell_position]; }
		...
		constexpr zone_ct_t<T, RegionSize * ZoneSize, ZoneBorder> compile() const noexcept {
			zone_ct_t<T, RegionSize * ZoneSize, ZoneBorder> zone{};
		...
		constexpr void compile(ref<zone_ct_t<T, RegionSize * ZoneSize, ZoneBorder>> zone) const noexcept {
			for (extent_t::scalar_t region_y{ 0 }; region_y < region_size.h; ++region_y) {
				for (extent_t::scalar_t region_x{ 0 }; region_x < region_size.w; ++region_x) {
					const offset_t region_pos{ region_x, region_y };
					for (extent_t::scalar_t zone_y{ 0 }; zone_y < zone_size.h; ++zone_y) {
