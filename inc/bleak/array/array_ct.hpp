...
namespace bleak {
	template<typename T, extent_t Size> class array_ct_t {
	  private:
		ptr<T> data;
	  ...
	  public:
		...
		inline constexpr array_ct_t() : data{ new T[area] } {}

		inline constexpr array_ct_t(std::array<T, area> elements) : data{} {
		...
		inline constexpr explicit array_ct_t(std::array<U, area> elements) : data{} {
			for (usize i{ 0 }; i < area; ++i) {
				data[i] = elements[i];
			}
		}

		inline constexpr array_ct_t(std::initializer_list<T> elements) : data{} {
		...
		inline constexpr array_ct_t(cref<Params>... elements) : data{ new T[area] } {
			usize i{ 0 };
			((data[i++] = elements), ...);
		}
		...
		template<typename... Params>
			requires is_homogeneous<T, Params...>::value && is_plurary<Params...>::value && (sizeof...(Params) <= Size.size())
		inline constexpr array_ct_t(rval<Params>... elements) : data{ new T[area] } {
			usize i{ 0 };
			((data[i++] = std::move(elements)), ...);
		}

		inline constexpr array_ct_t(cref<array_ct_t> other) : data{ new T[area] } {
			for (usize i{ 0 }; i < area; ++i) {
				data[i] = other.data[i];
			}
		}

		inline constexpr array_ct_t(rval<array_ct_t> other) noexcept : data{ std::move(other.data) } { other.data = nullptr; }

		inline constexpr ref<array_ct_t> operator=(cref<array_ct_t> other) noexcept {
			if (this == &other) {
				return *this;
			}
		...
		inline constexpr ref<array_ct_t> operator=(rval<array_ct_t> other) noexcept {
			if (this == &other) {
				return *this;
			}
		...
		inline constexpr ~array_ct_t() noexcept {
			if (data != nullptr) {
				delete[] data;
				data = nullptr;
			}
		...
		constexpr bool operator==(cref<array_ct_t> other) const noexcept {
			for (usize i{0}; i < area; ++i) {
		...
		struct hasher {
			static constexpr usize operator()(cref<array_ct_t<T, Size>> array) { return hash_array(array.begin(), array.end()); }
		};
	};
} // namespace bleak
