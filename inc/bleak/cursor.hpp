...
namespace bleak {
	struct cursor_t {
	  ...
	  public:
		...
		inline cursor_t() : current_position{ 0, 0 }, color{ colors::White } {}

		inline cursor_t(color_t color) : current_position{ 0, 0 }, color{ color } {}

		inline void update() { current_position = mouse_s::get_position(); }

		inline void update(offset_t pos) { current_position = pos; };

		inline void draw(cref<texture_t> texture) const { texture.draw(current_position, color); }

		inline void draw(cref<texture_t> texture, offset_t offset) const { texture.draw(current_position + offset, color); }
	...
	template<extent_t Size> struct grid_cursor_t {
		...
		inline grid_cursor_t() : current_position{}, min{}, max{}, use_bounds{ false }, color{ colors::White } {}

		inline grid_cursor_t(color_t color) : current_position{}, min{}, max{}, use_bounds{ false }, color{ color } {}

		inline grid_cursor_t(offset_t min, offset_t max) : current_position{}, min{ min }, max{ max }, use_bounds{ true }, color{ colors::White } {}

		inline grid_cursor_t(color_t color, offset_t min, offset_t max) : current_position{}, min{ min }, max{ max }, use_bounds{ true }, color{ color } {}
		...
		inline void update(cref<camera_t> camera) {
			const offset_t grid_pos{ (mouse_s::get_position() - necrowarp::globals::SparseTileNudge - necrowarp::globals::grid_origin<necrowarp::grid_type_e::Game>()) / size + camera.get_position() };
		...
		inline void draw(cref<texture_t> texture) const { texture.draw(current_position * size, color); }

		inline void draw(cref<texture_t> texture, cref<camera_t> camera) const { texture.draw((current_position - camera.get_position()) * size, color); }

		inline void draw(cref<texture_t> texture, offset_t offset) const { texture.draw(current_position * size + offset, color); }

		inline void draw(cref<texture_t> texture, cref<camera_t> camera, offset_t offset) const { texture.draw((current_position - camera.get_position()) * size + offset, color); }
