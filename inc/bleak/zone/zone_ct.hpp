...
namespace bleak {
	template<typename T, extent_t RegionSize, extent_t ZoneSize, extent_t ZoneBorder = extent_t::Zero> struct region_ct_t;

	template<typename T, extent_t Size, extent_t BorderSize = extent_t{ 0, 0 }> class zone_ct_t {
	  ...
	  public:
		...
		constexpr zone_ct_t() : cells{} {}

		constexpr zone_ct_t(cref<std::string> path) : cells{} {
			std::ifstream file{};
		...
		constexpr zone_ct_t(cref<zone_ct_t<T, Size, BorderSize>> other) : cells{ other.cells } {};

		constexpr zone_ct_t(rval<zone_ct_t<T, Size, BorderSize>> other) : cells{ std::move(other.cells) } {}

		constexpr ref<zone_ct_t<T, Size, BorderSize>> operator=(cref<zone_ct_t<T, Size, BorderSize>> other) noexcept {
			if (this != &other) {
				cells = other.cells;
			}
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> operator=(rval<zone_ct_t<T, Size, BorderSize>> other) noexcept {
			if (this != &other) {
		...
		constexpr ~zone_ct_t() noexcept {}

		constexpr cref<array_ct_t<T, Size>> data() const noexcept { return cells; }

		constexpr cptr<array_ct_t<T, Size>> data_ptr() const noexcept { return &cells; }

		constexpr array_ct_t<cref<T>, Size> view() const {
			array_ct_t<cref<T>, Size> view{};
		...
		constexpr array_ct_t<ref<T>, Size> proxy() noexcept {
			array_ct_t<ref<T>, Size> proxy{};
		...
		constexpr array_ct_t<cref<T>, Size> proxy() const noexcept {
			array_ct_t<ref<T>, Size> proxy{};
		...
		template<zone_region_e Region> constexpr ref<zone_ct_t<T, Size, BorderSize>> set(cref<T> value) noexcept {
			if constexpr (Region == zone_region_e::All) {
				for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
					cells[i] = value;
				}
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> set(cref<U> value) noexcept {
			if constexpr (Region == zone_region_e::All) {
				for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
					cells[i] = value;
				}
		...
		template<zone_region_e Region> constexpr ref<zone_ct_t<T, Size, BorderSize>> reset() noexcept {
			set<Region>(T{});
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> apply(cref<T> value) noexcept {
			if constexpr (Region == zone_region_e::All) {
				for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
					cells[i] += value;
				}
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> apply(cref<U> value) noexcept {
			if constexpr (Region == zone_region_e::All) {
				for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
					assert(i < zone_area);
					cells[i] += value;
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> apply(cref<Params>... values) noexcept {
			if constexpr (Region == zone_region_e::All) {
				for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> repeal(cref<T> value) noexcept {
			if constexpr (Region == zone_region_e::All) {
				for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
					cells[i] -= value;
				}
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> repeal(cref<U> value) noexcept {
			if constexpr (Region == zone_region_e::All) {
				for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
					cells[i] -= value;
				}
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> repeal(cref<Params>... values) noexcept {
			if constexpr (Region == zone_region_e::All) {
				for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
					for (auto value : { values... }) {
						cells[i] -= value;
		...
		constexpr void swap(ref<array_ct_t<T, Size>> buffer) noexcept { std::swap(cells, buffer); }

		constexpr void sync(cref<array_ct_t<T, Size>> buffer) noexcept {
			for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
				cells[i] = buffer[i];
			}
		}
		...
		template<typename U>
			requires std::is_assignable<T, U>::value
		constexpr void sync(cref<array_ct_t<U, Size>> buffer) noexcept {
			for (extent_t::product_t i{ 0 }; i < zone_area; ++i) {
				cells[i] = buffer[i];
			}
		}
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> randomize(ref<Randomizer> generator) noexcept {
		...
		template<zone_region_e Region, typename Randomizer>
			requires is_random_engine<Randomizer>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> randomize(ref<Randomizer> generator, f64 fill_percent, cref<T> true_value, cref<T> false_value) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> randomize(ref<Randomizer> generator, f64 fill_percent, cref<U> true_value, cref<U> false_value) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer>
			requires is_random_engine<Randomizer>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> randomize(ref<Randomizer> generator, f64 fill_percent, cref<binary_applicator_t<T>> applicator) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer, typename U>
			requires is_random_engine<Randomizer>::value && std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> randomize(ref<Randomizer> generator, f64 fill_percent, cref<binary_applicator_t<U>> applicator) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer>
			requires is_random_engine<Randomizer>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> randomize(ref<Randomizer> generator, f64 fill_percent, cref<binary_applicator_t<T>> applicator, cref<std::vector<offset_t>> spokes) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer, typename U>
			requires is_random_engine<Randomizer>::value && std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> randomize(ref<Randomizer> generator, f64 fill_percent, cref<binary_applicator_t<U>> applicator, cref<std::vector<offset_t>> spokes) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region> constexpr ref<zone_ct_t<T, Size, BorderSize>> spoke(cref<T> value, cref<std::vector<offset_t>> spokes) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		constexpr ref<zone_ct_t<T, Size, BorderSize>> spoke(cref<U> value, cref<std::vector<offset_t>> spokes) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region> constexpr cref<zone_ct_t<T, Size, BorderSize>> spoke(ref<array_ct_t<T, Size>> buffer, cref<T> value, cref<std::vector<offset_t>> spokes) const noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename U>
			requires is_equatable<T, U>::value && std::is_assignable<T, U>::value
		constexpr cref<zone_ct_t<T, Size, BorderSize>> spoke(ref<array_ct_t<T, Size>> buffer, cref<U> value, cref<std::vector<offset_t>> spokes) const noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region> constexpr ref<zone_ct_t<T, Size, BorderSize>> collapse(cref<T> value, usize index, cref<T> collapse_to) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}

			array_ct_t<T, Size> buffer{ cells };
		...
		template<zone_region_e Region, typename U>
			requires is_equatable<T, U>::value && std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> collapse(cref<U> value, usize index, cref<U> collapse_to) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region> constexpr ref<zone_ct_t<T, Size, BorderSize>> collapse(ref<array_ct_t<T, Size>> buffer, cref<T> value, usize index, cref<T> collapse_to) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename U>
			requires is_equatable<T, U>::value && std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> collapse(ref<array_ct_t<T, Size>> buffer, cref<U> value, usize index, cref<U> collapse_to) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<bool Safe = false> constexpr void modulate(ref<array_ct_t<T, Size>> buffer, offset_t position, u8 threshold, cref<T> true_state, cref<T> false_state) const noexcept {
		...
		template<bool Safe = false, typename U>
			requires std::is_assignable<T, U>::value
		constexpr void modulate(ref<array_ct_t<T, Size>> buffer, offset_t position, u8 threshold, cref<U> true_state, cref<U> false_state) const noexcept {
			u8 neighbours{ neighbour_count<Safe>(position, true_state) };
		...
		template<bool Safe = false> constexpr void modulate(ref<array_ct_t<T, Size>> buffer, offset_t position, u8 threshold, cref<binary_applicator_t<T>> applicator) const noexcept {
		...
		template<bool Safe = false, typename U>
			requires std::is_assignable<T, U>::value
		constexpr void modulate(ref<array_ct_t<T, Size>> buffer, offset_t position, u8 threshold, cref<binary_applicator_t<U>> applicator) const noexcept {
			u8 neighbours{ neighbour_count<Safe>(position, applicator.true_value) };
		...
		template<zone_region_e Region> constexpr cref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u8 threshold, cref<T> true_value, cref<T> false_state) const noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename U>
			requires std::is_assignable<T, U>::value
		constexpr cref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u8 threshold, cref<U> true_value, cref<U> false_state) const noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region> constexpr cref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u8 threshold, cref<binary_applicator_t<T>> applicator) const noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename U>
			requires std::is_assignable<T, U>::value
		constexpr cref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u8 threshold, cref<binary_applicator_t<U>> applicator) const noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region> constexpr ref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u32 iterations, u8 threshold, cref<T> true_value, cref<T> false_state) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename U>
			requires std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u32 iterations, u8 threshold, cref<U> true_value, cref<U> false_state) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region> constexpr ref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u32 iterations, u8 threshold, cref<binary_applicator_t<T>> applicator) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename U>
			requires std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u32 iterations, u8 threshold, cref<binary_applicator_t<U>> applicator) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region> constexpr ref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u32 iterations, u8 threshold, cref<binary_applicator_t<T>> applicator, cref<std::vector<offset_t>> spokes) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename U>
			requires std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> automatize(ref<array_ct_t<T, Size>> buffer, u32 iterations, u8 threshold, cref<binary_applicator_t<U>> applicator, cref<std::vector<offset_t>> spokes) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer>
			requires is_random_engine<Randomizer>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<T> true_value, cref<T> false_state) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer, typename U>
			requires is_random_engine<Randomizer>::value && std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<U> true_value, cref<U> false_state) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer>
			requires is_random_engine<Randomizer>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<binary_applicator_t<T>> applicator) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer, typename U>
			requires is_random_engine<Randomizer>::value && std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<binary_applicator_t<U>> applicator) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer>
			requires is_random_engine<Randomizer>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<binary_applicator_t<T>> applicator, cref<std::vector<offset_t>> spokes) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer, typename U>
			requires is_random_engine<Randomizer>::value && std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<binary_applicator_t<U>> applicator, cref<std::vector<offset_t>> spokes) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer>
			requires is_random_engine<Randomizer>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<array_ct_t<T, Size>> buffer, ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<T> true_value, cref<T> false_state) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer, typename U>
			requires is_random_engine<Randomizer>::value && std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<array_ct_t<T, Size>> buffer, ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<U> true_value, cref<U> false_state) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer>
			requires is_random_engine<Randomizer>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<array_ct_t<T, Size>> buffer, ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<binary_applicator_t<T>> applicator) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region, typename Randomizer, typename U>
			requires is_random_engine<Randomizer>::value && std::is_assignable<T, U>::value
		constexpr ref<zone_ct_t<T, Size, BorderSize>> generate(ref<array_ct_t<T, Size>> buffer, ref<Randomizer> generator, f64 fill_percent, u32 iterations, u8 threshold, cref<binary_applicator_t<U>> applicator) noexcept {
			if constexpr (Region == zone_region_e::None) {
				return *this;
			}
		...
		template<zone_region_e Region> constexpr void linear_apply(ref<array_ct_t<T, Size>> buffer, offset_t origin, offset_t target, cref<T> value) const noexcept {
			if (!within<Region>(origin) || !within<Region>(target)) {
				return;
			}
		...
		template<zone_region_e Region, typename U>
			requires std::is_assignable<T, U>::value
		constexpr void linear_apply(ref<array_ct_t<T, Size>> buffer, offset_t origin, offset_t target, cref<U> value) const noexcept {
			if (!within<Region>(origin) || !within<Region>(target)) {
				return;
			}
