...
template<typename T> struct ForwardIterator
{
    ...
    constexpr ref<ForwardIterator> operator++() { ++handle; return *this; }
    constexpr ref<ForwardIterator> operator++(int) { auto temp{ *this }; ++handle; return temp; }

    constexpr ref<ForwardIterator> operator--() { --handle; return *this; }
    constexpr ref<ForwardIterator> operator--(int) { auto temp{ *this }; --handle; return temp; }

    constexpr friend bool operator==(cref<ForwardIterator> lhs, cref<ForwardIterator> rhs) { return lhs.handle == rhs.handle; }
    constexpr friend bool operator!=(cref<ForwardIterator> lhs, cref<ForwardIterator> rhs) { return lhs.handle != rhs.handle; }
...
template<typename T> struct ReverseIterator
{
    ...
    constexpr ref<ReverseIterator> operator++() { --handle; return *this; }
    constexpr ref<ReverseIterator> operator++(int) { auto temp{ *this }; --handle; return temp; }

    constexpr ref<ReverseIterator> operator--() { ++handle; return *this; }
    constexpr ref<ReverseIterator> operator--(int) { auto temp{ *this }; ++handle; return temp; }

    constexpr friend bool operator==(cref<ReverseIterator> lhs, cref<ReverseIterator> rhs) { return lhs.handle == rhs.handle; }
    constexpr friend bool operator!=(cref<ReverseIterator> lhs, cref<ReverseIterator> rhs) { return lhs.handle != rhs.handle; }
...
public:
    ...
    static inline constexpr usize flatten(cref<Point<uhalf>> position) { return position.y * Width + position.x; }

    static inline constexpr Point<uhalf> unflatten(usize index) { return { index % Width, index / Width }; }
    static inline constexpr void unflatten(usize index, ref<uhalf> i, ref<uhalf> j) { i = index % Width; j = index / Width; }
...
#pragma region Iterators
    ...
    constexpr const_reverse_iterator rbegin(uhalf j) const { return const_reverse_iterator(data + (j + 1) * Width - 1); }
    constexpr const_reverse_iterator rend(uhalf j) const { return const_reverse_iterator(data + j * Width - 1); }

    constexpr const_reverse_iterator crbegin(uhalf j) const { return const_reverse_iterator(data + (j + 1) * Width - 1); }
